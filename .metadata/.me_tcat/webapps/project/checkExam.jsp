  ...
  tool.teacherTool tt=(tool.teacherTool)session.getAttribute("teacher");
   ...
   %>
    ...
    border-bottom:1px solid">
          ...
          text-align:center;font-size:16px"><b>欢迎，教师，<%=tt.getT().getTname()%></b></div>
     ...
     <%//班级导航连接 %>
         ...
         solid #aaaaaa">
             ...
             font-size:14px;display:block;">><a href="checkExam.jsp">作业详情</a></div>
           </div>
	...
	rowcount=tt.getCurrenet().size();
	 // System.out.println(rowcount);  
	pagecount=(rowcount+pagesize-1)/pagesize;
    int i=thepage;
        while(i<(pagesize+thepage)&&rowcount!=0&&i<tt.getCurrenet().size())
         {     
         %>
         <%if(i%2==1)
         {
          %>
            <tr bgcolor="#cccccc"><td width="15%" style="padding-left:450px;padding-top:15px"><%=i+1 +". "+tt.getCurrenet().get(i).getContext() %></td>
             <td width="40%"style="padding-top:15px">=<%=tt.getCurrenet().get(i).getAnswer()%></td>
          <%i++;}else{ %>
        
         <tr><td width="15%" style="padding-left:450px;padding-top:15px"><%=i+1 +". "+tt.getCurrenet().get(i).getContext() %></td>
             <td width="40%"style="padding-top:15px">=<%=tt.getCurrenet().get(i).getAnswer()%></td>
             <%i++;} %>
         ...
         <% } %>
               ...
               <% if(check!=null){if(check.equals("1")){%><a href="Tcreateexam2.jsp">返回</a><%}else{%><input type="submit" value="提交" ><%} }else{%><a href="Tbandetail.jsp">返回</a> <%}%>
               </div>
           </form>
        </div>
             </div>
